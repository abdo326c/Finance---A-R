...
def get_retroactive_scholarship_tx(db_session, student_id, term, academic_year, sch_type_id, sch_name, requested_pct, current_counter, batch_id=None):
    ...
    net_billed = db_session.query(func.sum(Transaction.debit - Transaction.credit)).filter(
        Transaction.student_id == student_id, Transaction.term == term,
        Transaction.academic_year == academic_year, Transaction.transaction_type.in_(tuition_types)
    ).scalar() or 0.0
    ...
    existing_other_txs = db_session.query(Transaction.description).filter(
        Transaction.student_id == student_id, Transaction.term == term,
        Transaction.academic_year == academic_year, Transaction.reference_no.like('SCH-%'),
    ...
    existing_discount = db_session.query(func.sum(Transaction.credit - Transaction.debit)).filter(
        Transaction.student_id == student_id, Transaction.term == term,
        Transaction.academic_year == academic_year, Transaction.scholarship_type_id == sch_type_id,
    ...
    if abs(diff) > 0.01:
        ...
        if actual_pct < requested_pct: desc += f" (Capped from {requested_pct}%)"
        tx = Transaction(
...
if 'sch_lookup_params' not in st.session_state: st.session_state['sch_lookup_params'] = None
if 'view_doc_id' not in st.session_state: st.session_state['view_doc_id'] = None
...
tab_search, tab_reg, tab1, tab2, tab3, tab_sch, tab_batch, tab_docs, tab4 = st.tabs([
    ...
    "📤 Bulk Financials", "🎓 Scholarships", "🗑️ Batch Management", "📚 Policies & Docs", "📈 Management Reports"
])
...
with tab_search:
        ...
        with get_db() as db:
            ...
            if student:
                    ...
                    with st.form("status_update_form"):
                        ...
                        if st.form_submit_button("💾 Save Status"):
                            ...
                            if existing_status:
                                ...
                                db.add(StudentStatus(student_id=search_lookup_id, term=stat_term, academic_year=stat_year, status=stat_value))
                ...
                student_schs_all = db.query(StudentScholarship, ScholarshipType).join(
                    ScholarshipType, StudentScholarship.scholarship_type_id == ScholarshipType.id
                ).filter(StudentScholarship.student_id == search_lookup_id, StudentScholarship.is_active == True).order_by(StudentScholarship.academic_year.desc(), StudentScholarship.term).all()
                
                if student_schs_all:
                    df_schs = pd.DataFrame([{"Term": ss.term, "Year": ss.academic_year, "Scholarship": st_type.name, "Percentage": f"{ss.percentage * 100 if ss.percentage <= 1 else ss.percentage:.1f}%"} for ss, st_type in student_schs_all])
                    st.dataframe(df_schs, use_container_width=True, hide_index=True)
                else: st.info("No active scholarships found for this student.")
                ...
                if edit_mode:
                    ...
                    if st.button("💾 Save Changes", type="primary"):
                        try:
                            student.name = e_name; student.college = e_college.upper(); student.price_per_hr = e_price
                            student.email = e_email; student.mobile = e_mobile; student.program = e_program
                            db.commit(); st.session_state['flash_msg'] = "✅ Student master data updated successfully!"; st.rerun()
    ...
    for i, stat in enumerate(VALID_STATUSES): cols[i].code(stat, language="text")

    with st.expander("🛠️ Open Bulk Status Updater"):
        stat_template = {"Student ID": 26100123, "Term": VALID_TERMS[1], "Year": DEFAULT_YEAR, "Status": VALID_STATUSES[-1]}
        buf_stat = io.BytesIO()
                ...
                with get_db() as db:
                    ...
                    for _, r in df_stat.iterrows():
                        ...
                        if sid <= 0 or sid not in valid_ids:
                            row_dict['Error Reason'] = "Student ID Not Registered"; failed_records.append(row_dict); continue
                        if not status_val:
                            row_dict['Error Reason'] = f"Invalid Status. Allowed: {', '.join(VALID_STATUSES)}"; failed_records.append(row_dict); continue
                        if trm not in VALID_TERMS or yr <= 0:
                            row_dict['Error Reason'] = f"Invalid Term or Year. Term must be {', '.join(VALID_TERMS)}."; failed_records.append(row_dict); continue
                    ...
                    if success_count > 0:
                        try:
                            db.commit(); st.success(f"✅ Successfully updated {success_count} student statuses!")
                        except Exception as e:
                            db.rollback(); st.error(f"❌ Database Error: {e}")
    ...
    if st.button("🚀 Load Fast Export"):
        with st.spinner("Compiling High-Speed Export..."):
            sql_query = """SELECT id AS "Student ID", name AS "Full Name", college AS "College Code", program AS "Program", price_per_hr AS "Price Per Hour (EGP)", email AS "University Email", mobile AS "Mobile Number", national_id AS "National ID", nationality AS "Nationality", birth_date AS "Birth Date", admit_year AS "Admit Year" FROM students"""
            df_all_students = pd.read_sql(sql_query, con=engine)
            buf_all = io.BytesIO()
            df_all_students.to_excel(buf_all, index=False)
            st.download_button(label="📥 Download Excel File", data=buf_all.getvalue(), file_name=f"NU_Students_MasterData_{datetime.now().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
                ...
                if n_id is None or not n_name or not n_college:
                    ...
                    with get_db() as db:
                        if db.query(Student).filter_by(id=n_id).first(): st.error("⚠️ Student ID already exists!")
                            ...
                            try:
                                db.add(Student(id=n_id, name=n_name, college=n_college.upper(), program=n_program, price_per_hr=n_price, email=n_email, mobile=n_mobile, national_id=n_nat_id, nationality=n_nationality, admit_year=n_admit, birth_date=n_dob))
                                db.commit(); st.session_state['flash_msg'] = f"✅ Student '{n_name}' has been registered!"; st.rerun()
                            except Exception as e:
                                db.rollback(); st.error(f"❌ Error: {e}")
    else:
            ...
            with st.spinner("Registering students..."):
                ...
                with get_db() as db:
                    ...
                    for _, r in df_std.iterrows():
                        ...
                        row_dict = r.to_dict()
                        if sid <= 0: row_dict['Error Reason'] = "Invalid Student ID"; failed_records.append(row_dict); continue
                        if sid in existing_ids: row_dict['Error Reason'] = "Student ID already exists"; failed_records.append(row_dict); continue
                            
                        bd_dt = pd.to_datetime(r.get('Birth Date'), errors='coerce')
                        new_students.append(Student(id=sid, name=str(r.get('Name', 'Unknown')), college=str(r.get('College', 'N/A')).upper(), program=str(r.get('Program', '')), price_per_hr=float(r.get('Price Per Hr', 0.0)), email=str(r.get('Email', '')), mobile=str(r.get('Mobile', '')), national_id=str(r.get('National ID', '')), nationality=str(r.get('Nationality', 'Egyptian')), admit_year=int(r.get('Admit Year', DEFAULT_YEAR)), birth_date=bd_dt.date() if pd.notna(bd_dt) else None))
                        success_count += 1
                        
                    if new_students:
                        try:
                            db.add_all(new_students); db.commit(); st.success(f"✅ Successfully registered {success_count} new students!")
                        except Exception as e:
                            db.rollback(); st.error(f"❌ Database Error: {e}"); success_count = 0
                    elif success_count == 0 and not failed_records: st.warning("⚠️ No data found in the uploaded file.")
                        
                    if failed_records:
                        st.error(f"⚠️ {len(failed_records)} records failed or skipped. See report below.")
                        df_failed = pd.DataFrame(failed_records)
                        st.dataframe(df_failed, use_container_width=True)
                        buf_err = io.BytesIO(); df_failed.to_excel(buf_err, index=False)
            ...
            with get_db() as db:
                ...
                if b_t == "Update Student Rates":
                    for _, r in df_b.iterrows():
                        ...
                        if r_id <= 0:
                            original_row['Error Reason'] = "Invalid Student ID"; failed_records.append(original_row)
                        elif r_id not in rts:
                            original_row['Error Reason'] = "Student ID not registered"; failed_records.append(original_row)
                        else:
                            db.query(Student).filter(Student.id == r_id).update({"price_per_hr": float(r['New_Price_Per_Hr'])})
                            success_count += 1
                            
                    if success_count > 0:
                        db.commit(); st.success(f"✅ Successfully updated rates for {success_count} students!")
                else:
                    m_id = get_next_ref_sequence(db)
                    bulk_l = []
                    tx_counter = 1
                    ...
                    for i, r in df_b.iterrows():
                        ...
                        if sid <= 0: original_row['Error Reason'] = "Invalid Student ID"; failed_records.append(original_row); continue
                        if sid not in rts: original_row['Error Reason'] = "Student ID not registered"; failed_records.append(original_row); continue
                        ...
                        if b_t == "Bulk Payments":
                            pfx, cr = "PAY", float(str(r.get('Amount', 0)).replace(',', '').strip() or 0.0)
                            ...
                            pfx, dr = "INV", float(str(r.get('Fee Amount', 0)).replace(',', '').strip() or 0.0)
                            ...
                            pfx, v, dsc, h_change = "ADJ", abs(h * rt), f"Adj {h} CH", h
                            dr = v if h > 0 else 0
                            cr = v if h < 0 else 0
...
with tab_sch:
    ...
    if sch_action == "View / Edit":
        ...
        if st.session_state.get('sch_lookup_params') and st.session_state['sch_lookup_params']['sid'] > 0:
            ...
            with get_db() as db:
                ...
                if not student: st.warning("⚠️ Student not found.")
                    ...
                    student_schs = db.query(StudentScholarship, ScholarshipType).join(
                        ScholarshipType
                    ).filter(
                            ...
                            with st.expander(f"⚙️ Manage '{st_type.name}' Mode"):
                                if ss.is_active:
                                    if st.button("Stop Future Only", key=f"stop_fut_{ss.id}"):
                                        ss.is_active = False; db.commit(); st.rerun()
            ...
            if st.form_submit_button("➕ Add Scholarship"):
                with get_db() as db:
                    if not db.query(Student).filter_by(id=add_sid).first(): st.error("❌ Student not found!")
                        ...
                        st.session_state['flash_msg'] = "✅ Processed successfully!"; st.rerun()
        ...
        st.download_button(label="📥 Download Template", data=io.BytesIO(pd.DataFrame([sch_template]).to_excel(index=False)), file_name="Template_Scholarships.xlsx")
            ...
            with st.spinner("Processing..."):
                ...
                with get_db() as db:
                    ...
                    for _, r in df_sch.iterrows():
                        ...
                        if sid <= 0 or sid not in valid_ids or not s_type_id or pct <= 0: failed_records.append(r.to_dict()); continue
                    ...
                    if retro_txs: db.bulk_save_objects(retro_txs); db.commit()
        ...
        if st.button("📂 Generate Scholarship Report"):
            with st.spinner("Compiling comprehensive report..."):
                sql = text("""
                    SELECT 
                        s.id AS "Student ID", s.name AS "Student Name", s.college AS "College", ss.term AS "Term", ss.academic_year AS "Year", st.name AS "Scholarship Name", ss.percentage AS "Configured %", 
                ...
                if not df_rep.empty:
                    ...
                    buf = io.BytesIO(); df_rep.to_excel(buf, index=False)
...
with tab_batch:
    ...
    with get_db() as db:
        ...
        if batch_action == "📂 View Active Batches":
                ...
                if st.button("📥 Load Batch"):
                    ...
                    if not sel_batch: st.warning("Please enter a valid Batch ID.")
                        ...
                        if not df_ex.empty:
                            ...
                            buf = io.BytesIO(); df_ex.to_excel(buf, index=False)
                        ...
                        else: st.error("❌ Batch ID not found. Please make sure you copied it correctly from the Active Batches list.")
                ...
                if batch_summary:
                    with st.form("del_batch", clear_on_submit=True):
                        ...
                        if st.form_submit_button("🗑️ Delete Batch"):
                            b_del = b_del.strip()
                            if not b_del: st.error("⚠️ Please enter the Batch ID you want to delete.")
                            elif not confirm_checkbox: st.error("⚠️ You must check the confirmation box to proceed.")
                            else:
                                recs = [b for b in batch_summary if b.batch_id == b_del]
                                if not recs: st.error(f"❌ Batch ID '{b_del}' not found! Please make sure you typed or copied it correctly.")
                                else:
                                    try:
                                        ...
                                        db.commit(); st.session_state['flash_msg'] = f"✅ Successfully deleted batch {b_del}."; st.rerun()
                                    except Exception as e:
                                        db.rollback(); st.error(f"❌ Error: {e}")
            else: st.error("🔒 **Access Denied**: Only Finance Admins can delete batches.")
            ...
            if deleted_logs:
                st.dataframe(pd.DataFrame([{"Batch ID": l.batch_id, "Type(s)": l.transaction_type, "Records": l.record_count, "Total Debit": f"{l.total_debit:,.2f}", "Total Credit": f"{l.total_credit:,.2f}", "Deleted By": l.deleted_by, "Deleted At": l.deleted_at.strftime('%Y-%m-%d %H:%M:%S')} for l in deleted_logs]), use_container_width=True)
            ...
            if docs:
                    ...
                    with st.container():
                        ...
                        if st.session_state.get('logged_in_user') == 'fin_admin':
                            if c4.button("🗑️ Delete", key=f"del_{doc.id}"):
                                db.delete(doc); db.commit(); st.rerun()
                        else: c4.write(" ") 
                ...
                if st.session_state.get('view_doc_id'):
                    ...
                    if doc_to_view: 
                        ...
                        if st.button("❌ Close Document Reader"): st.session_state['view_doc_id'] = None; st.rerun()
                    ...
                    if not doc_title or not doc_file: st.error("⚠️ Title and PDF file are required.")
                        ...
                        with get_db() as db:
                            try:
                                ...
                                db.commit(); st.session_state['flash_msg'] = f"✅ Document uploaded!"; st.rerun()
                            except Exception as e:
                                db.rollback(); st.error(f"❌ Upload failed: {e}")
    ...
    if st.session_state.get('report_params'):
        ...
        with st.spinner("Processing High-Speed Data Report..."):
            ...
            if p['format'] == "Student Academic Status Report":
                sql = text("""
                    SELECT s.id AS "Student ID", s.name AS "Student Name", s.college AS "College", s.program AS "Program", ss.term AS "Term", ss.academic_year AS "Year", ss.status AS "Academic Status"
                    FROM student_statuses ss JOIN students s ON ss.student_id = s.id 
                    WHERE (:c_cnt = 0 OR s.college IN :cls) AND (:t_cnt = 0 OR ss.term IN :trms) AND (:y_cnt = 0 OR ss.academic_year IN :yrs) AND (:s_cnt = 0 OR ss.status IN :stats)
                ...
                params = {"c_cnt": len(p['col']), "cls": tuple(p['col']) if p['col'] else ('',), "t_cnt": len(p['term']), "trms": tuple(p['term']) if p['term'] else ('',), "y_cnt": len(p['year']), "yrs": tuple(p['year']) if p['year'] else (-1,), "s_cnt": len(p['status']), "stats": tuple(p['status']) if p['status'] else ('',)}
                df = pd.read_sql(sql, con=engine, params=params)
                if not df.empty: st.dataframe(df.style.format({"Price/Hr": "{:,.2f}", "Reg. Hours": "{:,.1f}", "Tuition Billed": "{:,.2f}", "Other Fees": "{:,.2f}", "Discounts": "{:,.2f}", "Payments": "{:,.2f}", "Adjustments": "{:,.2f}", "Balance": "{:,.2f}"}), use_container_width=True)
                ...
                if len(p['dates']) != 2: st.warning("⚠️ Please select a Date Range.")
                    ...
                    sql = text("""
                        SELECT s.id AS "ID", s.name AS "Student Name", s.college AS "College", 
                            COALESCE((SELECT status FROM student_statuses WHERE student_id = s.id ORDER BY id DESC LIMIT 1), 'Not Set') AS "Current Status", COALESCE(SUM(t.hours_change), 0) AS "CH Changed", 
                            COALESCE(SUM(CASE WHEN t.transaction_type IN ('Invoice', 'Bulk Invoices (Tuition)') THEN t.debit ELSE 0 END), 0) AS "Tuition Billed", 
                            COALESCE(SUM(CASE WHEN t.transaction_type IN ('Other Fees', 'Bulk Other Fees') THEN t.debit ELSE 0 END), 0) AS "Other Fees", 
                    ...
                    df = pd.read_sql(sql, con=engine, params=params)
                    if not df.empty: st.dataframe(df.style.format({"CH Changed": "{:,.1f}", "Tuition Billed": "{:,.2f}", "Other Fees": "{:,.2f}", "New Discounts": "{:,.2f}", "Payments Received": "{:,.2f}", "Adjustments": "{:,.2f}", "Net Period Change": "{:,.2f}"}), use_container_width=True)
                ...
                df = pd.read_sql(sql, con=engine, params=params)
                if not df.empty: st.dataframe(df.style.format({"Hours": "{:,.1f}", "Debit": "{:,.2f}", "Credit": "{:,.2f}"}), use_container_width=True)
